...
def interval_gradient_algorithm(f , f_grad , x0 , infbound , supbound , step , PREC , ITE_MAX ):
    ...
    for k in range(ITE_MAX):
        ...
        ## update iterate
        # TODO: complete by gradient step projected onto the set "a <= x <= b"
        g = f_grad(x)
        x = x
...
### Adaptive stepsize projected gradient algorithm for interval constraints

import numpy as np
import timeit

def adaptive_gradient_algorithm(f , f_grad , x0 , infbound , supbound , step , PREC , ITE_MAX ):
    x = np.copy(x0)
    stop = PREC*np.linalg.norm(f_grad(x))
    ...
    x_tab = np.copy(x)
    print("------------------------------------\n Adaptive Stepsize gradient\n------------------------------------\nSTART    -- stepsize = {:0}".format(step))
    t_s =  timeit.default_timer()
    for k in range(ITE_MAX):

        ## update iterate
        # TODO: complete by gradient step projected onto the set "a <= x <= b"
        # TODO: find a "good" stepsize at each iteration
        g = f_grad(x)
        x = x
